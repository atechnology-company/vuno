...
#[tauri::command]
pub async fn search_web(query: String, api_key: Option<String>, perplexity_store: Option<tauri::State<'_, PerplexityKeyStore>>) -> Result<PerplexitySearchResponse, String> {
    // Get API key from parameter or store
    let key = if let Some(k) = api_key {
        k
    } else if let Some(store) = perplexity_store {
        store.get_key()
    } else {
